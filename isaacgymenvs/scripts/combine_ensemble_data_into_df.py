...
def tflog2pandas(path: str) -> pd.DataFrame:
    ...
    try:
        ...
        for tag in tags:
            tag = str(tag)
            keywords = tag.split('/')
            if not "generator" in keywords[-1]:
                continue
            metric = ""
            for j in range(len(keywords) - 2):
                metric += keywords[j] + "/"
            metric += keywords[-2]
            exp_name = keywords[-1] 
            ...
            r = {"experiment": [exp_name] * len(step), "metric": [metric] * len(step), "value": values, "step": step}
            r = pd.DataFrame(r)
    ...
    for path in event_paths:
        ...
        if log is not None:
            if all_logs.shape[0] == 0:
                ...
                all_logs = pd.concat([all_logs, log], ignore_index=True)
    return all_logs


def find_all_files(dir_path: str, files_found: List[str] = []) -> List[str]:
    assert os.path.isdir(dir_path), "Given path is not a directory"

    new_files_found = []
    for a in os.scandir(dir_path):
        a_path = os.path.join(dir_path, a)
        if os.path.isdir(a_path):
            new_files_found += find_all_files(a_path, files_found)
        if os.path.isfile(a_path) and "events" in a_path:
            new_files_found.append(a_path)
    
    return files_found + new_files_found
...
@click.option("--out-dir", "-o", help="output directory", default=os.path.dirname(__file__))
@click.option("--filename", "-f", help="output filename", default="all_training_logs_in_one_file")
def main(logdir_or_logfile: str, write_pkl: bool, write_csv: bool, out_dir: str, filename: str):
    ...
    if os.path.isdir(logdir_or_logfile):
        ...
        event_paths = find_all_files(logdir_or_logfile)
